...
class RefundAccountHandler
{
    ...
    /**
     * RefundAccountHandler constructor.
     ...
     */
    public function __construct(GuidGeneratorInterface $guidFactory, TransactionRepository $transactionRepository, MessageFactory $messageFactory, AMessageFactory $aMessageFactory, RMessageFactory $rMessageFactory, IMessageFactory $iMessageFactory, MessageBusInterface $appBus, MessageBusInterface $mysqlBus, SerializerInterface $serializer, $notificationFactory, Notifier $notifier)
    {
     ...
     * @inheritDoc
     */
    function __invoke(RefundAccount $command)
    {
        ...
        $agreementOwnerBillingProfile = $this->mysqlBus->dispatch($aQuery); // mysqlBus

        // Create agreement owner transaction (Credit)
        $agreementOwnerTransaction = $this->serializer->denormalize(array(
            ...
            'description' => Transaction::DESC_REFUND,
            'aNetTransId' => '',
        ...
        try {
            ...
            // Create referrer transaction (Debit 60% of agreement bid)
